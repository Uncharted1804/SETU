...
class ModelPlanner:
    ...
    def _system_prompt(self) -> str:
        ...
        lines += [
            ...
            "  * DO NOT schedule 'kb_search', 'read_file' or 'list_dir' unless the task text or an",
            "    attached file actually references something you need to look up - an existing",
            "    document, a prior report, a project file, or domain knowledge not given in the",
            "    task. A self-contained request that already states everything needed to answer",
            "    it (e.g. 'write a python script that prints X', 'implement bubble sort',",
            "    'what is 12 * 7') needs NO retrieval step at all - go straight to the agent.",
            "  * The entry router below already chose which agent this task needs. Start your",
            "    plan with that agent unless the task text gives you a concrete, stated reason",
            "    to do otherwise (e.g. it also needs a file read first). Do not add steps",
            "    'just in case' - every extra step is an extra approval and an extra chance to fail.",
            "  * Propose the SHORTEST plan that actually satisfies the request. Propose between",
            "    1 and %d steps." % self.MAX_STEPS,
