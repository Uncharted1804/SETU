...
class SetuService:
    ...
    def create_task(self, req: TaskCreateRequest) -> tuple[TaskRecord, RouterDecision, Planner]:
        ...
        # Ids first: the task root is needed to materialise uploads, and the
        # envelope must be built with the REWRITTEN paths rather than mutated
        # afterwards.
        task_id = new_task_id()
        session_id = req.session_id or new_session_id()
        file_paths = self._materialise_uploads(task_id, session_id, list(req.file_paths))
